#!/usr/bin/env python3
"""
FR5 运动控制节点 V12 —— 轨迹接收版本
fairino SDK + blendT 平滑过渡 + SystemStateMachine + 回中 + HostComm + TrajectoryServer

核心改动（V11 → V12）：
  · 引入 traj_server.TrajectoryServer，监听轨迹模块的 TCP 推送（端口 9001）
  · FR5Controller 新增 execute_trajectory()，执行轨迹模块发来的逐点轨迹
    逻辑链与 _execute_move 完全对齐：状态机检查、stop响应、误差计算、
    CSV日志、comm上报、trigger_error，缺一不补
  · main() 中统一管理 traj_srv 的启动与关闭
...
# blendT 配置（关键参数）
BLEND_MID  = 40.0  # 中间插值点的过渡时间（ms），越大越平滑但精度略低
BLEND_END  = -1.0   # 最后一个点，-1=阻塞到位，保证精度
...
class FR5Controller:
    ...
    # 底层 MoveJ（可指定 blendT）
    # ──────────────────────────────────────────
    def _movej(self, joint_list: list, blend_t: float = BLEND_END) -> int:
        """
        blend_t = -1.0  → 阻塞到位（用于最后一个点）
        blend_t = 200.0 → 200ms过渡，不硬停（用于中间插值点）
        """
        try:
            ret = self.robot.MoveJ(
                list(map(float, joint_list)),
                0,
                0,
    ...
    def stop_motion(self):
        """
        立即停止当前运动，丢弃剩余路径（不可 resume 续跑）。
        StopMove() 走 send_message 独立 TCP（8080端口），
        真正抢占正在执行的 MoveJ，不受主连接指令队列影响。
        与 hold 的区别：stop 丢弃路径；hold 可 resume 续跑。
        状态机功能态（AUTO/MANUAL/ASSIST）保持不变。
        """
        if not self._is_moving:
    ...
    def _execute_move(self, target: list, smooth: bool,
        ...
        mode_str = "平滑blendT" if smooth else "直接到位"

        print(f"    目标(°): {[round(v,3) for v in target]}")
        print(f"    当前(°): {[round(v,3) for v in current]}")
        print(f"    预计时间: {duration:.2f}s  模式:{mode_str}  速度:{SDK_SPEED}%")

        self._is_moving = True
        self._stop_event.clear()          # 每次运动开始前复位停止事件
        t_start      = time.time()
        ok           = False
        error_reason = ""
        ...
        try:
            if smooth:
                steps     = max(5, int(INTERP_STEPS * duration / 2.0))
                ...
                for i, wp in enumerate(waypoints):
                    ...
                    # 全部插值点用 BLEND_MID（非阻塞）：
                    # MoveJ 发出指令后立即返回，线程不阻塞，
                    # stop_event 可在任意两步之间被响应，彻底消除线程卡死。
                    ret = self._movej(wp, blend_t=BLEND_MID)
                    if ret != 0:
                        if self._stop_event.is_set():
                            error_reason = "stop指令中断"
                        else:
                            error_reason = f"MoveJ错误码:{ret} (第{i+1}段)"
                        break
                else:
                    # 所有插值点已发完，轮询等待机械臂真正到位（可被 stop 打断）
                ...
                # 直接模式（非平滑）：blendT=-1 阻塞，stop 靠 StopMove 硬件打断
                ret = self._movej(target, blend_t=BLEND_END)
                if self._stop_event.is_set():
                    error_reason = "stop指令中断"
                elif ret != 0:
                    error_reason = f"MoveJ错误码:{ret}"
                else:
                    ok = True
            ...
            self._paused    = False   # 运动结束，暂停标志复位
            # StopMove 后机械臂进入错误锁定，ResetAllError 恢复可用
    ...
    def execute_trajectory(self, payload: dict) -> bool:
        """
        执行轨迹模块推送过来的轨迹。
        直接按轨迹模块给的 t/q 逐点执行，不套 s_curve（尊重轨迹模块的时序规划）。
        逻辑链与 _execute_move 完全对齐：
          · 状态机检查（can_move）
          · 逐点 MoveJ，中间点 BLEND_MID，末点 BLEND_END（阻塞到位）
          · stop / estop / 状态机异常 → 立即中断
          · 末点到位等待（轮询误差 < 1°）
          · 误差计算、CSV 日志、comm 上报、状态机 trigger_error
          · stop 后 ResetAllError
        """
        with self._move_lock:
            if not self._check_can_move():
                return False
        ...
        target   = [math.degrees(float(v)) for v in points[-1]["q"]]   # 末点用于误差计算（rad→deg）
        ...
        try:
            for i, pt in enumerate(points):
                ...
                blend   = BLEND_END if is_last else BLEND_MID

                ret = self._movej(q, blend_t=blend)
                if ret != 0:
                    if self._stop_event.is_set():
                        error_reason = "stop指令中断"
                    else:
                        error_reason = f"MoveJ错误码:{ret} (第{i+1}点)"
                    break
...
def print_help():
    ...
    print("    help / q")
    print()
    print(f"  当前blendT配置: 中间点={BLEND_MID}ms  终点={BLEND_END}")
    print("="*60)
